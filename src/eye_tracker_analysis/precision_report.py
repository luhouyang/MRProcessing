...
# Configuration
SESSIONS_ROOT_DIR = r'C:\Users\luhou\Desktop\python\MRProcessing\data\experiment_filtered'
# SESSIONS_ROOT_DIR = r"C:\Users\luhou\Desktop\python\MRProcessing\data\ARCHIVE\TEST3"
...
def analyze_data_frequency(df_in, viz_output_folder):
    ...
    if not all(col in df_in.columns for col in required_cols):
        print(f"Warning: Skipping frequency analysis. Missing required column: {required_cols}", file=sys.stderr)
        return None
    ...
    report_filename = os.path.join(viz_output_folder, 'analysis_report_frequency.txt')
    try:
    ...
    fig.suptitle(f'Data Acquisition Rate Analysis - {os.path.basename(viz_output_folder)}', y=1.02)

    # Plot 1: Inter-Sample Interval (ISI) in milliseconds
    if not isi_data.empty:
        ax1.boxplot(isi_data, vert=False, whis=[5, 95], showfliers=False, showmeans=True)
        mean_val = np.nanmean(isi_data)
        ax1.text(mean_val, 1.1, f'Mean: {mean_val:.2f} ms', 
                 verticalalignment='bottom', horizontalalignment='center',
                 color='blue', fontsize=9, fontweight='bold')
        ax1.set_title('Inter-Sample Interval (ISI) [5th-95th Percentile]')
        ax1.set_xlabel('Time (milliseconds)'), ax1.set_yticklabels(['ISI (ms)'])
    else:
        ax1.set_title('No ISI Data')

    # Plot 2: Instantaneous Frequency (Hz)
    if not freq_data.empty:
        ax2.boxplot(freq_data, vert=False, whis=[5, 95], showfliers=False, showmeans=True)
        mean_val = np.nanmean(freq_data)
        ax2.text(mean_val, 1.1, f'Mean: {mean_val:.1f} Hz', 
                 verticalalignment='bottom', horizontalalignment='center',
                 color='blue', fontsize=9, fontweight='bold')
    ...
    plt.tight_layout()
    plot_filename = os.path.join(viz_output_folder, 'analysis_plot_frequency_boxplot.png')
    try:
...
def generate_open3d_geometries(selected_df, block_id, viz_output_folder):
    ...
    avg_origin = selected_df[['eyeOriginX', 'eyeOriginY', 'eyeOriginZ']].mean().values
    avg_target = selected_df[['globaltargetX', 'globaltargetY', 'globaltargetZ']].mean().values
    target_dist = np.linalg.norm(avg_target - avg_origin)
    
    if target_dist == 0:
        print(f"Warning: Target distance is 0 for Block {block_id}. Using 1.0.")
    ...
    gaze_vec_sel = ptk.normalize(selected_df[['eyeDirectionX', 'eyeDirectionY', 'eyeDirectionZ']].values)
    ...
    gaze_colors = [[0, 0.8, 0] for _ in range(len(gaze_lines_indices))] # Green
    ...
    o3d.io.write_line_set(os.path.join(viz_output_folder, f"gaze_cone_block_{block_id}.ply"), gaze_lineset)
    ...
    o3d.io.write_line_set(os.path.join(viz_output_folder, f"target_vector_block_{block_id}.ply"), target_lineset)
    ...
    o3d.io.write_triangle_mesh(os.path.join(viz_output_folder, f"eye_origin_block_{block_id}.ply"), eye_sphere)
    
    target_sphere = o3d.geometry.TriangleMesh.create_sphere(radius=sphere_radius)
    target_sphere.paint_uniform_color([1, 0, 0]) # Red
    target_sphere.translate(avg_target)
    o3d.io.write_triangle_mesh(os.path.join(viz_output_folder, f"target_point_block_{block_id}.ply"), target_sphere)
...
def analyze_head_movement(df_in, viz_output_folder):
    ...
    """    
    required_cols = ['headX', 'headY', 'headZ', 'headForwardX', 'headForwardY', 'headForwardZ', 'timestamp']
    if not all(col in df_in.columns for col in required_cols):
        print(f"Warning: Skipping head movement analysis. Missing one or more columns: {required_cols}", file=sys.stderr)
        return None
    ...
    report_filename = os.path.join(viz_output_folder, 'analysis_report_head_movement.txt')
    try:
        with open(report_filename, 'w') as f:
            f.write(report)
    except Exception as e:
    ...
    # 4. Save Box Plot
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 8), sharex=False)
    fig.suptitle(f'Head Movement Velocity - {os.path.basename(viz_output_folder)}', y=1.02)

    if not pos_vel_data.empty:
        ax1.boxplot(pos_vel_data, vert=False, whis=[5, 95], showfliers=False, showmeans=True)
        mean_val = np.nanmean(pos_vel_data)
        ax1.text(mean_val, 1.1, f'Mean: {mean_val:.3f}', 
                 verticalalignment='bottom', horizontalalignment='center',
                 color='blue', fontsize=9, fontweight='bold')
    ...
    if not ang_vel_data.empty:
        ax2.boxplot(ang_vel_data, vert=False, whis=[5, 95], showfliers=False, showmeans=True)
        mean_val = np.nanmean(ang_vel_data)
        ax2.text(mean_val, 1.1, f'Mean: {mean_val:.3f}', 
                 verticalalignment='bottom', horizontalalignment='center',
                 color='blue', fontsize=9, fontweight='bold')
    ...
    plt.tight_layout()
    plot_filename = os.path.join(viz_output_folder, 'analysis_plot_head_movement_boxplot.png')
    try:
        plt.savefig(plot_filename)
        plt.close(fig)
    except Exception as e:
    ...
    try:
        CHUNK_SIZE = 10
        path_chunks = [] # Store (x_start, y_start, z_start, x_end, y_end, z_end, velocity)
        ...
        for i in range(0, len(df_head) - CHUNK_SIZE, CHUNK_SIZE):
            ...
            x_start, y_start, z_start = start_row['headX'], start_row['headY'], start_row['headZ']
            x_end, y_end, z_end = end_row['headX'], end_row['headY'], end_row['headZ']
            distance = np.sqrt((x_end - x_start)**2 + (y_end - y_start)**2 + (z_end - z_start)**2)
            velocity = distance / dt
            
            if pd.notna(velocity):
                path_chunks.append((x_start, y_start, z_start, x_end, y_end, z_end, velocity))
        ...
        for x_start, y_start, z_start, x_end, y_end, z_end, vel in path_chunks:
            color = cmap(norm(vel))
            ax_drift.plot([x_start, x_end], [y_start, y_end], [z_start, z_end], color=color, alpha=0.7, linewidth=2)
            
        first_x, first_y, first_z = path_chunks[0][0], path_chunks[0][1], path_chunks[0][2]
        ax_drift.plot([first_x], [first_y], [first_z], 'r+', markersize=15, label=f'Start Position ({first_x:.2f}, {first_y:.2f}, {first_z:.2f})')
        
        fig_drift.colorbar(plt.cm.ScalarMappable(norm=norm, cmap=cmap), 
                           ax=ax_drift, label='Average Velocity (m/s)', shrink=0.8)
        
        ax_drift.set_title(f'Head Movement Path 3D ({CHUNK_SIZE}-sample chunks) - {os.path.basename(viz_output_folder)}')
        ax_drift.set_xlabel('Head X Position (m)'), ax_drift.set_ylabel('Head Y Position (m)'), ax_drift.set_zlabel('Head Z Position (m)')
        ax_drift.legend()
        ...
        max_range /= 2.0
        mid_x, mid_y, mid_z = (max_x+min_x)/2.0, (max_y+min_y)/2.0, (max_z+min_z)/2.0
        ...
        plot_drift_filename = os.path.join(viz_output_folder, 'analysis_plot_head_movement_path_3D.png')
        plt.savefig(plot_drift_filename)
        plt.close(fig_drift)
    
    except Exception as e:
        print(f"Error generating head movement 3D path plot: {e}", file=sys.stderr)
...
def analyze_eye_to_gaze_distance(df_in, viz_output_folder):
    ...
    """    
    required_cols = ['eyeOriginX', 'eyeOriginY', 'eyeOriginZ', 'globalX', 'globalY', 'globalZ']
    if not all(col in df_in.columns for col in required_cols):
        print(f"Warning: Skipping eye-to-gaze-hit analysis. Missing one or more columns: {required_cols}", file=sys.stderr)
        return None
    ...
    report_filename = os.path.join(viz_output_folder, 'analysis_report_eye_gaze_distance.txt')
    try:
        with open(report_filename, 'w') as f:
            f.write(report)
    except Exception as e:
    ...
    try:
        ...
        ax.scatter(data_x, data_y, alpha=0.1, s=10, label='Eye-to-Gaze-Hit Vector (XY Plane)')
        ax.plot(0, 0, 'r+', markersize=10, label='Eye Origin (0,0)')

        circle_mean = plt.Circle((0, 0), mean_dist_3d, color='r', fill=False, linewidth=2, label=f'Mean 3D Dist: {mean_dist_3d:.2f}m')
        circle_std_plus = plt.Circle((0, 0), mean_dist_3d + std_dist_3d, color='k', fill=False, linestyle='--', alpha=0.7, label=f'Mean +/- 1 STD')
        ax.add_patch(circle_mean)
        ax.add_patch(circle_std_plus)
        if (mean_dist_3d - std_dist_3d) > 0:
            circle_std_minus = plt.Circle((0, 0), mean_dist_3d - std_dist_3d, color='k', fill=False, linestyle='--', alpha=0.7)
            ax.add_patch(circle_std_minus)

        ax.set_title(f'Eye-to-Gaze-Hit Vector (XY) & 3D Distance - {os.path.basename(viz_output_folder)}')
        ax.set_xlabel('X Distance (Gaze - Eye Origin) (m)'), ax.set_ylabel('Y Distance (Gaze - Eye Origin) (m)')
        ax.axis('equal'), ax.grid(True, linestyle='--', alpha=0.6), ax.legend()
        
        plot_filename = os.path.join(viz_output_folder, 'analysis_plot_eye_gaze_bullseye.png')
        plt.savefig(plot_filename)
        plt.close(fig)

    except Exception as e:
        print(f"Error generating eye-to-gaze-hit bullseye plot: {e}", file=sys.stderr)
...
def analyze_gaze_data_3D(df_gaze_in, output_path, model_obj_path, viz_output_folder, window_size=30):
    """
    ...
    overall_best_df = ptk.find_stable_windows(df_gaze, 'angular_error_deg', window_size)

    if overall_best_df.empty:
        print(f"Error: Could not find any valid windows for {folder_basename}. Aborting analysis.", file=sys.stderr)
        return None
    ...
    for block_id, block_df in overall_best_df_viz.groupby('block_id'):
        gaze_lineset = generate_open3d_geometries(block_df, block_id, viz_output_folder)
        if gaze_lineset:
    ...
    if all_gaze_linesets:
        ...
        o3d.io.write_line_set(os.path.join(viz_output_folder, "gaze_cone_ALL_COMBINED.ply"), combined_lineset)

    # 4. Matplotlib Plots & Report
    # 4a. Get Stats & Report (Based on all best windows)
    stats = ptk.summarize_metrics(overall_best_df, ['cosine_similarity', 'angular_error_deg', 'distance_2d'])
    # Get stats for precision (mean of all blocks' precision)
    if df_precision is not None and not df_precision.empty:
        precision_stats = ptk.summarize_metrics(df_precision, ['precision_rms_3d', 'precision_std_3d', 'precision_rms_2d', 'precision_std_2d'])
    else:
    ...
    fig_box, (ax_box1, ax_box2, ax_box3, ax_box4, ax_box5) = plt.subplots(5, 1, figsize=(10, 18), sharex=False) # Changed to 5, 1
    fig_box.suptitle(f'3D Gaze Error & Precision (Best Windows) - {folder_basename}', y=1.02)
    ...
    precision_rms_3d_data = df_precision['precision_rms_3d'].dropna() if df_precision is not None else pd.Series(dtype='float64')
    precision_std_3d_data = df_precision['precision_std_3d'].dropna() if df_precision is not None else pd.Series(dtype='float64')
    
    if not ang_err_data.empty:
        ax_box1.boxplot(ang_err_data, vert=False, whis=[5, 95], showfliers=False, showmeans=True)
        mean_val = np.nanmean(ang_err_data)
        ax_box1.text(mean_val, 1.1, f'Mean: {mean_val:.3f}', 
                     verticalalignment='bottom', horizontalalignment='center',
                     color='blue', fontsize=9, fontweight='bold')
        ax_box1.set_title('Angular Error (deg) [5th-95th Percentile]')
        ax_box1.set_xlabel('Error (degrees)'), ax_box1.set_yticklabels(['Angular Error'])
    else: ax_box1.set_title('No Angular Error Data')

    if not cos_sim_data.empty:
        ax_box2.boxplot(cos_sim_data, vert=False, whis=[5, 95], showfliers=False, showmeans=True)
        mean_val = np.nanmean(cos_sim_data)
        ax_box2.text(mean_val, 1.1, f'Mean: {mean_val:.3f}', 
                     verticalalignment='bottom', horizontalalignment='center',
                     color='blue', fontsize=9, fontweight='bold')
        ax_box2.set_title('Cosine Similarity [5th-95th Percentile]')
        ax_box2.set_xlabel('Similarity'), ax_box2.set_yticklabels(['Cosine Sim.'])
    else: ax_box2.set_title('No Cosine Similarity Data')

    if not dist_2d_data.empty:
        ax_box3.boxplot(dist_2d_data, vert=False, whis=[5, 95], showfliers=False, showmeans=True)
        mean_val = np.nanmean(dist_2d_data)
        ax_box3.text(mean_val, 1.1, f'Mean: {mean_val:.3f}', 
                     verticalalignment='bottom', horizontalalignment='center',
                     color='blue', fontsize=9, fontweight='bold')
        ax_box3.set_title('2D Distance Error (units) [5th-95th Percentile]')
        ax_box3.set_xlabel('Error (units)'), ax_box3.set_yticklabels(['2D Dist. Error'])
    else: ax_box3.set_title('No 2D Distance Data')

    # Precision Plots
    if not precision_rms_3d_data.empty:
        ax_box4.boxplot(precision_rms_3d_data, vert=False, whis=[5, 95], showfliers=False, showmeans=True)
        mean_val = np.nanmean(precision_rms_3d_data)
        ax_box4.text(mean_val, 1.1, f'Mean: {mean_val:.3f}', 
                     verticalalignment='bottom', horizontalalignment='center',
                     color='blue', fontsize=9, fontweight='bold')
        ax_box4.set_title('Angular Precision 3D (RMS from Centroid) [5th-95th Percentile of blocks] (deg)')
        ax_box4.set_xlabel('Precision (deg)'), ax_box4.set_yticklabels(['RMS 3D (deg)'])
    else: ax_box4.set_title('No 3D RMS Precision Data')
        
    if not precision_std_3d_data.empty:
        ax_box5.boxplot(precision_std_3d_data, vert=False, whis=[5, 95], showfliers=False, showmeans=True)
        mean_val = np.nanmean(precision_std_3d_data)
        ax_box5.text(mean_val, 1.1, f'Mean: {mean_val:.3f}', 
                     verticalalignment='bottom', horizontalalignment='center',
                     color='blue', fontsize=9, fontweight='bold')
        ax_box5.set_title('Angular Precision 3D (STD from Centroid) [5th-95th Percentile of blocks] (deg)')
        ax_box5.set_xlabel('Precision (deg)'), ax_box5.set_yticklabels(['STD 3D (deg)'])
    else: ax_box5.set_title('No 3D STD Precision Data')


    plt.tight_layout()
    plot_box_filename = os.path.join(viz_output_folder, 'analysis_plot_3D_error_boxplot.png')
    ...
    gaze_vec_sel = ptk.normalize(df_plot_ud[['globalX', 'globalY', 'globalZ']].values - df_plot_ud[['eyeOriginX', 'eyeOriginY', 'eyeOriginZ']].values)
    ...
    plt.scatter(error_az, error_el, alpha=0.7, label=f'Gaze Point Error (All {n_windows} Best Windows)')
    plt.plot(0, 0, 'r+', markersize=10, label='Target (0, 0)')
    plt.title(f'2D Gaze Error (UD Model) - {folder_basename}')
    plt.xlabel('Azimuth Error (degrees)'), plt.ylabel('Elevation Error (degrees)')
    plt.grid(True, linestyle='--', alpha=0.5), plt.legend(), plt.axis('equal')
    plot_2d_filename = os.path.join(viz_output_folder, 'analysis_plot_2D_UD_model.png')
    ...
    unique_targets = df_gaze[['globaltargetX', 'globaltargetY', 'globaltargetZ']].drop_duplicates()
    ax.scatter(unique_targets['globaltargetX'], unique_targets['globaltargetY'], unique_targets['globaltargetZ'], 
               c='r', marker='x', s=100, label='All Targets (from pointcloud.csv)')
    
    ax.scatter(overall_best_df_viz['eyeOriginX'], overall_best_df_viz['eyeOriginY'], overall_best_df_viz['eyeOriginZ'], 
               c='b', marker='.', s=10, alpha=0.3, label='Eye Origins (Best Windows)')

    for block_id, df_window in overall_best_df_viz.groupby('block_id'):
        gaze_hit_points = df_window[['globalX', 'globalY', 'globalZ']].values
        ax.scatter(gaze_hit_points[:, 0], gaze_hit_points[:, 1], gaze_hit_points[:, 2], 
                   c='g', marker='o', s=10, alpha=0.3)

    ax.scatter([], [], [], c='g', marker='.', s=10, label='Gaze Hit Points (Best Windows)')
    ax.set_xlabel('X Coordinate (Global)'), ax.set_ylabel('Y Coordinate (Global)'), ax.set_zlabel('Z Coordinate (Global)')
    ax.set_title(f'3D Gaze Analysis - {folder_basename}'), ax.legend()
    
    plot_3d_filename = os.path.join(viz_output_folder, 'analysis_plot_3D_scatter.png')
    plt.savefig(plot_3d_filename)
    plt.close(fig)
    
    # 4e. Copy the model.obj
    if os.path.exists(model_obj_path):
        shutil.copy(model_obj_path, os.path.join(viz_output_folder, "model.obj"))
    else:
        print(f"Warning: model.obj not found at {model_obj_path}", file=sys.stderr)
...
def analyze_gaze_data_3D_TRACKING(df_gaze_in, output_path, model_obj_path, viz_output_folder):
    """
    ...
    if overall_best_df.empty:
        print(f"Error: No valid data for {folder_basename}. Aborting analysis.", file=sys.stderr)
        return None
    ...
    fig_box, (ax_box1, ax_box2, ax_box3, ax_box4, ax_box5) = plt.subplots(5, 1, figsize=(10, 18), sharex=False) # Changed to 5, 1
    fig_box.suptitle(f'3D Gaze Error & Precision (Full Tracking Task) - {folder_basename}', y=1.02)
    
    ang_err_data = overall_best_df['angular_error_deg'].dropna()
    cos_sim_data = overall_best_df['cosine_similarity'].dropna()
    dist_2d_data = overall_best_df['distance_2d'].dropna()
    precision_rms_3d_data = df_precision['precision_rms_3d'].dropna() if df_precision is not None and not df_precision.empty else pd.Series(dtype='float64')
    precision_std_3d_data = df_precision['precision_std_3d'].dropna() if df_precision is not None and not df_precision.empty else pd.Series(dtype='float64')
    ...
    if not precision_rms_3d_data.empty:
        ...
        ax_box4.set_title('Angular Precision 3D (RMS from Centroid) [5th-95th Percentile of blocks] (deg)')
        ax_box4.set_xlabel('Precision (deg)'), ax_box4.set_yticklabels(['RMS 3D (deg)'])
    else: ax_box4.set_title('No 3D RMS Precision Data (Not applicable for Tracking)')
    ...
    if not precision_std_3d_data.empty:
        ...
        ax_box5.set_title('Angular Precision 3D (STD from Centroid) [5th-95th Percentile of blocks] (deg)')
        ax_box5.set_xlabel('Precision (deg)'), ax_box5.set_yticklabels(['STD 3D (deg)'])
    else: ax_box5.set_title('No 3D STD Precision Data (Not applicable for Tracking)')
    ...
    plt.figure(figsize=(10, 8))
    plt.scatter(error_az, error_el, alpha=0.7, s=5, label=f'Gaze Point Error (All {n_windows} Blocks)') # Reduced scatter size
    plt.plot(0, 0, 'r+', markersize=10, label='Target (0, 0)')
    plt.title(f'2D Gaze Error (UD Model) - {folder_basename}')
    plt.xlabel('Azimuth Error (degrees)'), plt.ylabel('Elevation Error (degrees)')
    plt.grid(True, linestyle='--', alpha=0.5), plt.legend(), plt.axis('equal')
    ...
    # 4d. 3D Scatter Plot
    print(f"     Generating 3D scatter plot for {folder_basename}...")
    fig = plt.figure(figsize=(10, 8))
    ax = fig.add_subplot(111, projection='3d')
    ax.view_init(elev=20, azim=60, vertical_axis='y') 
    ...
    ax.scatter(overall_best_df_viz['eyeOriginX'], overall_best_df_viz['eyeOriginY'], overall_best_df_viz['eyeOriginZ'], 
               c='b', marker='.', s=10, alpha=0.3, label='Eye Origins (All Data)')
    ...
    ax.scatter([], [], [], c='g', marker='.', s=10, label='Gaze Hit Points (All Data)')
    ax.set_xlabel('X Coordinate (Global)'), ax.set_ylabel('Y Coordinate (Global)'), ax.set_zlabel('Z Coordinate (Global)')
    ax.set_title(f'3D Gaze Analysis - {folder_basename}'), ax.legend()
...
def analyze_gaze_data_2D(df_gaze_in, output_path, viz_output_folder, window_size=30):
    """
    ...
    stats = ptk.summarize_metrics(overall_best_df, ['distance_2d', 'angular_error_deg'])
    # Get stats for precision (mean of all blocks precision)
    if df_precision is not None and not df_precision.empty:
        precision_stats = ptk.summarize_metrics(df_precision, ['precision_rms_3d', 'precision_std_3d', 'precision_rms_2d', 'precision_std_2d'])
    else:
    ...
    fig_box_2d, (ax_box1, ax_box2, ax_box3, ax_box4) = plt.subplots(4, 1, figsize=(10, 15), sharex=False) # Changed to 4, 1
    fig_box_2d.suptitle(f'Gaze Error & Precision (Best Windows) - {folder_basename}', y=1.02)
    
    dist_2d_data = overall_best_df['distance_2d'].dropna()
    ang_err_data = overall_best_df['angular_error_deg'].dropna()
    # Precision data
    precision_rms_2d_data = df_precision['precision_rms_2d'].dropna() if df_precision is not None else pd.Series(dtype='float64')
    precision_std_2d_data = df_precision['precision_std_2d'].dropna() if df_precision is not None else pd.Series(dtype='float64')

    if not dist_2d_data.empty:
        ax_box1.boxplot(dist_2d_data, vert=False, whis=[5, 95], showfliers=False, showmeans=True)
        mean_val = np.nanmean(dist_2d_data)
        ax_box1.text(mean_val, 1.1, f'Mean: {mean_val:.3f}', 
                     verticalalignment='bottom', horizontalalignment='center',
                     color='blue', fontsize=9, fontweight='bold')
        ax_box1.set_title('2D Distance Error (units) [5th-95th Percentile]')
        ax_box1.set_xlabel('Error (units)'), ax_box1.set_yticklabels(['2D Dist. Error'])
    else: ax_box1.set_title('No 2D Distance Data')

    if not ang_err_data.empty:
        ax_box2.boxplot(ang_err_data, vert=False, whis=[5, 95], showfliers=False, showmeans=True)
        mean_val = np.nanmean(ang_err_data)
        ax_box2.text(mean_val, 1.1, f'Mean: {mean_val:.3f}', 
                     verticalalignment='bottom', horizontalalignment='center',
                     color='blue', fontsize=9, fontweight='bold')
        ax_box2.set_title('Angular Error (deg) [5th-95th Percentile]')
        ax_box2.set_xlabel('Error (degrees)'), ax_box2.set_yticklabels(['Angular Error'])
    else: ax_box2.set_title('No Angular Error Data')

    # Precision Plots
    if not precision_rms_2d_data.empty:
        ax_box3.boxplot(precision_rms_2d_data, vert=False, whis=[5, 95], showfliers=False, showmeans=True)
        mean_val = np.nanmean(precision_rms_2d_data)
        ax_box3.text(mean_val, 1.1, f'Mean: {mean_val:.3f}', 
                     verticalalignment='bottom', horizontalalignment='center',
                     color='blue', fontsize=9, fontweight='bold')
        ax_box3.set_title('Angular Precision 2D (RMS from Centroid) [5th-95th Percentile of blocks] (deg)')
        ax_box3.set_xlabel('Precision (deg)'), ax_box3.set_yticklabels(['RMS 2D (deg)'])
    else: ax_box3.set_title('No 2D RMS Precision Data')
        
    if not precision_std_2d_data.empty:
        ax_box4.boxplot(precision_std_2d_data, vert=False, whis=[5, 95], showfliers=False, showmeans=True)
        mean_val = np.nanmean(precision_std_2d_data)
        ax_box4.text(mean_val, 1.1, f'Mean: {mean_val:.3f}', 
                     verticalalignment='bottom', horizontalalignment='center',
                     color='blue', fontsize=9, fontweight='bold')
        ax_box4.set_title('Angular Precision 2D (STD from Centroid) [5th-95th Percentile of blocks] (deg)')
        ax_box4.set_xlabel('Precision (deg)'), ax_box4.set_yticklabels(['STD 2D (deg)'])
    else: ax_box4.set_title('No 2D STD Precision Data')

    plt.tight_layout()
    plot_box_filename_2d = os.path.join(viz_output_folder, 'analysis_plot_2D_error_boxplot.png')
    plt.savefig(plot_box_filename_2d)
    plt.close(fig_box_2d)
            
    # 5. Finalize and Save 2D Scatter Plot
    plt.figure(figsize=(10, 10))
    plt.scatter(df_gaze['globalX'], df_gaze['globalY'], label='All Gaze Points (Global)', alpha=0.1, s=10, c='cyan')
    unique_targets = df_gaze[['globaltargetX', 'globaltargetY']].drop_duplicates()
    plt.scatter(unique_targets['globaltargetX'], unique_targets['globaltargetY'], label='Targets (from pointcloud.csv)', marker='x', c='red', s=100)
    plt.scatter(overall_best_df['globalX'], overall_best_df['globalY'], label=f'Best Windows ({n_windows} found)', alpha=0.8, s=20, c='blue')
    ...
    plot_filename = os.path.join(viz_output_folder, 'analysis_plot_2D_scatter.png')
    plt.savefig(plot_filename)
    plt.close()
...
def run_summary_analysis(all_results_data, summary_output_dir):
    ...
    try:
        summary_csv_path = os.path.join(summary_output_dir, 'summary_all_results.csv')
        df_summary.to_csv(summary_csv_path, index=False)
    except Exception as e:
        print(f"Error saving summary CSV: {e}", file=sys.stderr)

    plot_definitions = [
        ('freq_mean', 'Data Acquisition Rate MEAN (All Sessions)', 'Frequency (Hz)'),
        ('freq_std', 'Data Acquisition Rate STDEV (All Sessions)', 'Frequency (Hz)'),
        ('isi_mean', 'Inter-Sample Interval MEAN (All Sessions)', 'Time (ms)'),
        ('isi_std', 'Inter-Sample Interval STDEV (All Sessions)', 'Time (ms)'),
        ('eye_gaze_dist_mean', 'Eye-to-Gaze-Hit 3D Distance MEAN (All Sessions)', 'Distance (m)'),
        ('eye_gaze_dist_std', 'Eye-to-Gaze-Hit 3D Distance STDEV (All Sessions)', 'Distance (m)'),
        ('ang_err_std', '3D Angular Error STDEV (All Sessions)', 'Angular Error (deg)'),
        ('ang_err_mean', '3D Angular Error MEAN (All Sessions)', 'Angular Error (deg)'),
        ('dist_2d_std', '2D Distance Error STDEV (All Sessions)', 'Distance Error (units)'),
        ('dist_2d_mean', '2D Distance Error MEAN (All Sessions)', 'Distance Error (units)'),
        ('pos_vel_std', 'Head Positional Velocity STDEV (All Sessions)', 'Velocity (m/s)'),
        ('pos_vel_mean', 'Head Positional Velocity MEAN (All Sessions)', 'Velocity (m/s)'),
        ('ang_vel_std', 'Head Angular Velocity STDEV (All Sessions)', 'Velocity (deg/s)'),
        ('ang_vel_mean', 'Head Angular Velocity MEAN (All Sessions)', 'Velocity (deg/s)'),
        ('precision_rms_3d_mean', 'Spatial Precision 3D (RMS) MEAN (All Sessions)', 'Precision (deg)'),
        ('precision_std_3d_mean', 'Spatial Precision 3D (STD) MEAN (All Sessions)', 'Precision (deg)'),
        ('precision_rms_2d_mean', 'Spatial Precision 2D (RMS) MEAN (All Sessions)', 'Precision (deg)'),
        ('precision_std_2d_mean', 'Spatial Precision 2D (STD) MEAN (All Sessions)', 'Precision (deg)'),
    ]
    ...
    # def get_analysis_class(folder_name):
    #     if 'ObjectCone' in folder_name: return 'Cone'
    #     if 'UD0411' in folder_name: return 'UD0411'
    #     if 'PlaneWSW' in folder_name: return 'Plane (Tracking)'
    #     if 'Plane' in folder_name: return 'Plane (Static)'
    #     return 'Other'

    # df_summary['analysis_class'] = df_summary['analysis_folder'].apply(
    #     get_analysis_class)

    # for col, title, ylabel in plot_definitions:
    #     if col not in df_summary.columns:
    #         continue
    #     df_plot = df_summary[['analysis_class', col]].dropna()
    #     if df_plot.empty:
    #         continue

    #     classes = df_plot['analysis_class'].unique()
    #     classes.sort()
    #     if 'Other' in classes:
    #         classes = [c for c in classes if c != 'Other'] + ['Other']
    #     data_to_plot = [
    #         df_plot[df_plot['analysis_class'] == c][col].values
    #         for c in classes
    #     ]
    #     if not data_to_plot:
    #         continue

    #     fig, ax = plt.subplots(figsize=(max(10, len(classes) * 1.5), 7))
    #     ax.boxplot(data_to_plot,
    #                labels=classes,
    #                whis=[5, 95],
    #                showmeans=True,
    ...
    #     ax.set_title(title.replace("(All Sessions)", "(By Class)"))
    #     ax.set_ylabel(ylabel)
    #     ax.set_xlabel('Task Class')
    #     plt.grid(True, linestyle='--', alpha=0.6, axis='y'), plt.tight_layout()

    #     plot_filename = f"summary_plot_by_class_{col}.png"
    #     plot_path = os.path.join(summary_output_dir, plot_filename)
    #     try:
    #         plt.savefig(plot_path)
    #         plt.close(fig)
    #     except Exception as e:
    #         print(f"Error saving class summary plot '{plot_filename}': {e}",
    #               file=sys.stderr)


def run_summary_timeseries_analysis(all_session_dataframes, summary_output_dir):
    """
    ...
    for data in all_session_dataframes:
        ...
        grouped_dfs[folder_name].append({'dataframe': data['dataframe'], 'has_valid_target': data['has_valid_target']})

    for folder_name, data_list in grouped_dfs.items():
        
        metrics = ['angular_error_deg', 'head_pos_velocity_mps', 'head_ang_velocity_dps', 'eye_gaze_dist_3D']
        
        # 1. Calculate Resampled Stats
        plot_stats, overall_stats = ptk.resample_timeseries_data(data_list, metrics, n_points=1000)
        
        if not plot_stats or not overall_stats:
             print(f"No valid data to plot for {folder_name}, skipping.")
             continue
             
        has_valid_target_for_group = any(d['has_valid_target'] for d in data_list)
        ...
        try:
            ...
            fig.suptitle(f'Summary Timeseries Overview - {folder_name} (n={len(data_list)} sessions)', fontsize=16, y=1.02)
            ...
            if has_valid_target_for_group and 'angular_error_deg' in plot_stats:
                ...
                ax1.plot(time_base, mean, label='Mean Error', color='red', linewidth=2)
                ax1.fill_between(time_base, mean - std, mean + std, color='red', alpha=0.2, label='Mean +/- 1 STD (time)')

                overall_mean = overall_stats['angular_error_deg']['mean']
                overall_std = overall_stats['angular_error_deg']['std']
                if pd.notna(overall_mean): ax1.axhline(overall_mean, color='k', linestyle='-', linewidth=2, label=f'Overall Mean: {overall_mean:.2f}')
                if pd.notna(overall_std):
                    ax1.axhline(overall_mean + overall_std, color='k', linestyle='--', linewidth=1, label=f'Overall +/- 1 STD')
                    ax1.axhline(overall_mean - overall_std, color='k', linestyle='--', linewidth=1)
                ax1.set_ylabel('Gaze Ang. Error (deg)'), ax1.set_title('Gaze Angular Error vs. Time')
            else:
                ax1.text(0.5, 0.5, 'No Target Data Available', horizontalalignment='center', verticalalignment='center', transform=ax1.transAxes, fontsize=15, color='gray', alpha=0.6)
                ax1.set_title('Gaze Angular Error vs. Time (No Target)'), ax1.set_ylabel('Gaze Ang. Error (deg)')
            ...
            if 'head_pos_velocity_mps' in plot_stats:
                ...
                ax2.plot(time_base, mean, label='Mean Velocity', color='blue', linewidth=2)
                ax2.fill_between(time_base, mean - std, mean + std, color='blue', alpha=0.2, label='Mean +/- 1 STD (time)')
                overall_mean = overall_stats['head_pos_velocity_mps']['mean']
                overall_std = overall_stats['head_pos_velocity_mps']['std']
                if pd.notna(overall_mean): ax2.axhline(overall_mean, color='k', linestyle='-', linewidth=2, label=f'Overall Mean: {overall_mean:.2f}')
                if pd.notna(overall_std):
                    ax2.axhline(overall_mean + overall_std, color='k', linestyle='--', linewidth=1, label=f'Overall +/- 1 STD')
                    ax2.axhline(overall_mean - overall_std, color='k', linestyle='--', linewidth=1)
                ax2.set_ylabel('Head Pos. Vel. (m/s)'), ax2.set_title('Head Positional Velocity vs. Time')
            ...
            if 'head_ang_velocity_dps' in plot_stats:
                ...
                ax3.plot(time_base, mean, label='Mean Velocity', color='green', linewidth=2)
                ax3.fill_between(time_base, mean - std, mean + std, color='green', alpha=0.2, label='Mean +/- 1 STD (time)')
                overall_mean = overall_stats['head_ang_velocity_dps']['mean']
                overall_std = overall_stats['head_ang_velocity_dps']['std']
                if pd.notna(overall_mean): ax3.axhline(overall_mean, color='k', linestyle='-', linewidth=2, label=f'Overall Mean: {overall_mean:.2f}')
                if pd.notna(overall_std):
                    ax3.axhline(overall_mean + overall_std, color='k', linestyle='--', linewidth=1, label=f'Overall +/- 1 STD')
                    ax3.axhline(overall_mean - overall_std, color='k', linestyle='--', linewidth=1)
                ax3.set_ylabel('Head Ang. Vel. (deg/s)'), ax3.set_title('Head Angular Velocity vs. Time')
            ...
            if 'eye_gaze_dist_3D' in plot_stats:
                ...
                ax4.plot(time_base, mean, label='Mean Eye-Gaze Dist', color='purple', linewidth=2)
                ax4.fill_between(time_base, mean - std, mean + std, color='purple', alpha=0.2, label='Mean +/- 1 STD (time)')
                overall_mean = overall_stats['eye_gaze_dist_3D']['mean']
                overall_std = overall_stats['eye_gaze_dist_3D']['std']
                if pd.notna(overall_mean): ax4.axhline(overall_mean, color='k', linestyle='-', linewidth=2, label=f'Overall Mean: {overall_mean:.2f}')
                if pd.notna(overall_std):
                    ax4.axhline(overall_mean + overall_std, color='k', linestyle='--', linewidth=1, label=f'Overall +/- 1 STD')
                    ax4.axhline(overall_mean - overall_std, color='k', linestyle='--', linewidth=1)
                ax4.set_ylabel('Distance (m)'), ax4.set_title('Eye-to-Gaze-Hit Distance vs. Time')
            ax4.set_xlabel('Normalized Time (0.0 = start, 1.0 = end)'), ax4.grid(True, linestyle='--', alpha=0.5), ax4.legend(loc='upper right')

            plt.tight_layout(rect=[0, 0.03, 1, 0.98])
            plot_filename = os.path.join(summary_output_dir, f'summary_timeseries_{folder_name}.png')
            plt.savefig(plot_filename)
            plt.close(fig)
        
        except Exception as e:
            print(f"Error generating summary timeseries plot for {folder_name}: {e}", file=sys.stderr)


def generate_folder_timeseries_plot(df_gaze_in, folder_name, viz_output_folder, has_valid_target):
    """
    Generates a 4-panel plot for a folder using the toolkit for calculations.
    """    
    try:
        # 1. Calculate all 4 metrics
        df_plot = ptk.calculate_all_timeseries_metrics(df_gaze_in, has_valid_target)
        if df_plot.empty:
            print("Warning: Timeseries calculation returned no data.", file=sys.stderr)
            return
            
        df_plot['time_sec'] = df_plot['timestamp'] - df_plot['timestamp'].iloc[0]

        # 2. Create the Plot
        fig, (ax1, ax2, ax3, ax4) = plt.subplots(4, 1, figsize=(20, 15), sharex=True)
        fig.suptitle(f'Timeseries Overview - {folder_name}', fontsize=16, y=1.02)
        
        # Plot 1: Angular Error
        if has_valid_target:
            metric_data = df_plot['angular_error_deg'].dropna()
            ax1.plot(df_plot['time_sec'], df_plot['angular_error_deg'], label='Angular Error', color='red', alpha=0.7, linewidth=0.8)
            if not metric_data.empty:
                mean_val, std_val = metric_data.mean(), metric_data.std()
                ax1.axhline(mean_val, color='red', linestyle='-', linewidth=2, label=f'Mean: {mean_val:.2f}')
                ax1.axhline(mean_val + std_val, color='red', linestyle='--', linewidth=1, label=f'Std: {std_val:.2f}')
                ax1.axhline(mean_val - std_val, color='red', linestyle='--', linewidth=1)
            ax1.set_ylabel('Gaze Ang. Error (deg)'), ax1.set_title('Gaze Angular Error vs. Time')
        else:
            ax1.text(0.5, 0.5, 'No Target Data Available', horizontalalignment='center', verticalalignment='center', transform=ax1.transAxes, fontsize=15, color='gray', alpha=0.6)
            ax1.set_title('Gaze Angular Error vs. Time (No Target)'), ax1.set_ylabel('Gaze Ang. Error (deg)')
        ...
        ax2.plot(df_plot['time_sec'], df_plot['head_pos_velocity_mps'], label='Positional Velocity', color='blue', alpha=0.7, linewidth=0.8)
        if not metric_data.empty:
            mean_val, std_val = metric_data.mean(), metric_data.std()
            ax2.axhline(mean_val, color='blue', linestyle='-', linewidth=2, label=f'Mean: {mean_val:.2f}')
            ax2.axhline(mean_val + std_val, color='blue', linestyle='--', linewidth=1, label=f'Std: {std_val:.2f}')
            ax2.axhline(mean_val - std_val, color='blue', linestyle='--', linewidth=1)
        ax2.set_ylabel('Head Pos. Vel. (m/s)'), ax2.set_title('Head Positional Velocity vs. Time')
        ax2.grid(True, linestyle='--', alpha=0.5), ax2.legend(loc='upper right')

        # Plot 3: Head Angular Velocity
        metric_data = df_plot['head_ang_velocity_dps'].dropna()
        ax3.plot(df_plot['time_sec'], df_plot['head_ang_velocity_dps'], label='Angular Velocity', color='green', alpha=0.7, linewidth=0.8)
        if not metric_data.empty:
            mean_val, std_val = metric_data.mean(), metric_data.std()
            ax3.axhline(mean_val, color='green', linestyle='-', linewidth=2, label=f'Mean: {mean_val:.2f}')
            ax3.axhline(mean_val + std_val, color='green', linestyle='--', linewidth=1, label=f'Std: {std_val:.2f}')
            ax3.axhline(mean_val - std_val, color='green', linestyle='--', linewidth=1)
        ax3.set_ylabel('Head Ang. Vel. (deg/s)'), ax3.set_title('Head Angular Velocity vs. Time')
        ax3.grid(True, linestyle='--', alpha=0.5), ax3.legend(loc='upper right')

        # Plot 4: Eye-to-Gaze Distance
        metric_data = df_plot['eye_gaze_dist_3D'].dropna()
        ax4.plot(df_plot['time_sec'], df_plot['eye_gaze_dist_3D'], label='Eye-Gaze Distance', color='purple', alpha=0.7, linewidth=0.8)
        if not metric_data.empty:
            mean_val, std_val = metric_data.mean(), metric_data.std()
            ax4.axhline(mean_val, color='purple', linestyle='-', linewidth=2, label=f'Mean: {mean_val:.2f}')
            ax4.axhline(mean_val + std_val, color='purple', linestyle='--', linewidth=1, label=f'Std: {std_val:.2f}')
            ax4.axhline(mean_val - std_val, color='purple', linestyle='--', linewidth=1)
        ax4.set_ylabel('Distance (m)'), ax4.set_title('Eye-to-Gaze-Hit Distance vs. Time')
        ax4.set_xlabel('Time (seconds)'), ax4.grid(True, linestyle='--', alpha=0.5), ax4.legend(loc='upper right')

        plt.tight_layout(rect=[0, 0.03, 1, 0.98])
        
        plot_filename = os.path.join(viz_output_folder, 'analysis_plot_timeseries_overview.png')
        plt.savefig(plot_filename)
        plt.close(fig)
    
    except Exception as e:
        print(f"Error generating folder-wide timeseries plot: {e}", file=sys.stderr)
...
def main():
    if not os.path.isdir(SESSIONS_ROOT_DIR):
        print(f"Error: Sessions root directory not found: {SESSIONS_ROOT_DIR}", file=sys.stderr)
        print("Please update the 'SESSIONS_ROOT_DIR' variable at the top of the script.", file=sys.stderr)
        return
    ...
    for session_name in os.listdir(SESSIONS_ROOT_DIR):
        ...
        for folder_name in FOLDERS_TO_ANALYZE:
            ...
            if not os.path.exists(pointcloud_csv):
                print(f"Warning: Missing 'pointcloud.csv' in {folder_name}, skipping.")
                continue
            ...
            try:
                ...
                if df_gaze.empty:
                    print(f"Error: 'pointcloud.csv' in {folder_name} is empty. Skipping.")
                    continue

                has_valid_target = True
                if 'targetName' not in df_gaze.columns or df_gaze['targetName'].isnull().all() or (df_gaze['targetName'] == 'null').all():
                    has_valid_target = False
                    print(f"Info: No valid targets found for {folder_name} (targetName is null/NaN). Skipping target-based gaze analysis.")

                all_session_dataframes.append({
                    'session': session_name, 
                    'analysis_folder': folder_name, 
                    'dataframe': df_gaze.copy(),
                ...
                required_cols = [
                    ...
                    'globalX', 'globalY', 'globalZ', 'globaltargetX', 'globaltargetY', 'globaltargetZ',
                    'eyeOriginX', 'eyeOriginY', 'eyeOriginZ', 'eyeDirectionX', 'eyeDirectionY', 'eyeDirectionZ'
                ]
                missing_cols = [col for col in required_cols if col not in df_gaze.columns]
                if missing_cols:
                    print(f"Error: 'pointcloud.csv' in {folder_name} is missing required columns: {missing_cols}. Skipping.")
                    print("Please re-export your data with all global and head coordinate columns.")
                    continue

            except Exception as e:
                print(f"Error loading pointcloud.csv in {folder_name}: {e}. Skipping.")
                continue
            
            # Frequency Analysis
            freq_results = analyze_data_frequency(df_gaze.copy(), viz_output_folder)
            if freq_results:
                freq_results['session'] = session_name
                freq_results['analysis_folder'] = folder_name
                all_results_data.append(freq_results)
            
            head_results = analyze_head_movement(df_gaze.copy(), viz_output_folder)
            if head_results:
                head_results['session'] = session_name
                head_results['analysis_folder'] = folder_name
                all_results_data.append(head_results)

            eg_results = analyze_eye_to_gaze_distance(df_gaze.copy(), viz_output_folder)
            if eg_results:
            ...
            if has_valid_target:
                if folder_name in ['PlaneClose', 'PlaneMiddle', 'PlaneFar']:
                    gaze_results = analyze_gaze_data_2D(
                        df_gaze.copy(), output_path=folder_path,
                        viz_output_folder=viz_output_folder, window_size=WINDOW_SIZE
                    )
                elif folder_name in ['PlaneWSW']:
                    gaze_results = analyze_gaze_data_3D_TRACKING(
                        df_gaze.copy(), output_path=folder_path, 
                        model_obj_path=model_obj_path, viz_output_folder=viz_output_folder
                    )
                else:
            ...
            generate_folder_timeseries_plot(df_gaze.copy(), folder_name, viz_output_folder, has_valid_target)
    ...
    if all_results_data:
        ...
        run_summary_timeseries_analysis(all_session_dataframes, summary_output_dir)
